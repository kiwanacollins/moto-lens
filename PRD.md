...
### Primary Use Case
A mechanic enters a VIN → the app decodes the car → displays a realistic visual representation → mechanic taps parts → sees part details & spare part guidance.
...
7. Support 1–2 daily users with no scalability pressure
...
- ✅ Looks premium
- ✅ Feels professional
- ✅ Works in real garages (high contrast, glare-resistant)
...
3. On success → navigate to VIN input page
4. Invalid credentials → show error message

### 1️⃣ VIN Lookup & Validation

#### User Flow
1. User inputs VIN
2. VIN format validated (17 characters)
3. Submit → backend request
...
### 2️⃣ Pseudo-3D Car Viewer (CORE EXPERIENCE)

⚠️ **No real 3D models. No WebGL. No Three.js.**
...
#### Implementation
- 8–16 AI-generated images per car
...
### 3️⃣ AI Image Generation Flow (Automated)

Images are generated on-demand using **Gemini / Imagen**.

#### Workflow
```
VIN decoded → Car metadata known
...
### 4️⃣ Parts Interaction (Smart & Simple)

⚠️ **No complex 3D mesh mapping.**
...
## 🧠 Data Flow Architecture (MVP – No Database)
...
### 2️⃣ Part Identification Prompt
```
...
### 3️⃣ Spare Parts Summary Prompt
```
...
### 4️⃣ Component Compatibility Prompt
```
...
## 🧠 Why This Architecture Is the RIGHT MOVE
...
- **Expected usage:** 1–2 daily users
...
1. ✅ VIN decoded accurately for German vehicles
2. ✅ 360° car view loads smoothly on mobile
3. ✅ Part identification works via tap
4. ✅ AI output looks professional, not AI-generated
5. ✅ App installs as PWA on mobile devices
6. ✅ Total cost stays under $1,000
7. ✅ Works in typical garage lighting (dark mode)
